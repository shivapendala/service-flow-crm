...
class CanManageLead(BasePermission):
    """
    Fine-grained permission check for Lead Management:
    - Customer role: blocked entirely.
    - Support Agent: Read-only access.
    - Sales Agent: Create leads, Read all, Edit/Delete only if assigned to them or created by them.
    ...
    def has_permission(self, request, view):
        ...
        # Sales Agents and above can create
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.user.role == 'Sales Agent':
            if request.method in SAFE_METHODS:
                return True
            # Allow modification only if owner or assigned
